...
class Intus(bpy.types.Operator):
    ...
    prin : bpy.props.BoolProperty(
        name="Print Ready",
        description="Adjust model to be dimensions for printing",
        default=False,
    )

    twoD : bpy.props.BoolProperty(
        name="2D Model", description="2D positive flow model", default=False
    )

    pback : bpy.props.BoolProperty(
        name="Narrow",
        description="Moves first 2 verticies back to allow for more space",
        default=False,
    )

    spacefill : bpy.props.BoolProperty(
        name="Spacefilling",
        description="Modifies model to maximize covered area by displacing opposite sides",
        default=False,
    )

    len : bpy.props.IntProperty(
        name="Length",
        description="Length of Whole Structure",
        default=20,
        min=10,
        max=50,
    )
    ...
    angle : bpy.props.IntProperty(
        name="Angle",
        description="Initial angle of branching",
        default=90,
        min=30,
        max=150,
    )

    bran : bpy.props.IntProperty(
        name="Divisions",
        description="Number of times the structure divides",
        default=0,
        min=0,
        max=7,
    )
    ...
    reduction : bpy.props.FloatProperty(
        name="Reduction",
        description="Amount by which the distance between branches reduces",
        default=2.0,
        min=1.0,
        max=10.0,
    )

    inout : bpy.props.BoolProperty(
        name="Inlet/Outlet", description="Inlet and Outlet for Structure", default=False
    )

    boundBox : bpy.props.BoolProperty(
        name="Bounding Box", description="Bounding Box around Structure", default=False
    )

    vertRad : bpy.props.FloatProperty(
        name="Skin Radius",
        description="Radius of Skin Modifier on Each Vertex",
        default=0.25,
        min=0.1,
        max=0.7,
    )

    emurray : bpy.props.BoolProperty(
        name="""Equal Murray's Law""",
        description="""Apply Equal Tapering Murray's Law to Structure""",
        default=False,
    )

    rmurray : bpy.props.BoolProperty(
        name="""Random Murray's Law""",
        description="""Apply Random(inequal) Murray's Law to Structure""",
        default=False,
    )

    murrayNum : bpy.props.FloatProperty(
        name="""Murray's Exponent""",
        description="""Exponent for Murray's Law""",
        default=3.0,
        min=1.0,
        max=4.0,
    )
    ...
    def execute(self, context):
        ...
        scene.collection.objects.link(myobject)  # linking the object to the scene
        ...
        if self.boundBox is True:
            bpy.ops.view3d.snap_cursor_to_center()
            if not self.prin:
                bpy.ops.mesh.primitive_cube_add(size=length)
                if self.twoD is True:
                    bpy.context.object.scale = (1.25, 0.75, 0.25)
                else:
                    bpy.context.object.scale = (1.25, 0.75, 0.5)
        ...
        if self.emurray is True and self.rmurray is False:
        ...
        if self.rmurray is True and self.emurray is False:
        ...
        bpy.data.objects["ScriptedVessels"].select_set(True)
        bpy.context.view_layer.objects.active = bpy.data.objects["ScriptedVessels"]
        ...
        if self.spacefill:
            ...
            for f in range(3, iter):
                ...
                if z != 0:
                    if y > 0:
                        bpy.data.objects["ScriptedVessels"].data.vertices[f].co = (
                            x + 3,
                            y,
                            z,
                        )
        ...
        if self.inout:
            ...
            bpy.ops.object.select_all(action="DESELECT")
            bpy.data.objects["inout"].select_set(True)
            bpy.context.view_layer.objects.active = bpy.data.objects["inout"]
            bpy.ops.object.modifier_apply(apply_as="DATA", modifier="mirror")
